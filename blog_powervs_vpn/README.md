...
Find more information in the tutorial [Power Systems communication through a transit VPC](https://cloud.ibm.com/docs/solution-tutorials?topic=solution-tutorials-vpc-transit-power).  A primary concern is to carefully define the non overlapping CIDR blocks as shown in figure 2:
...
Open the [Virtual server instances for VPC](https://cloud.ibm.com/vpc-ext/compute/vs) and note the Enterprise Floating IP addresses which will be referred to as $EFIP later in the tutorial. Something like the following, but your IP address will be different.

![image](https://github.com/IBM-Cloud/vpc-transit/assets/6932057/8a9ff525-68a8-40bd-bcc3-0b86a024f05c)

``` sh
EFIP=169.62.21.234
```

It will be handy to keep in a temporary file to copy/paste into the shell in later steps.
...
1. Follow the initial instructions here [Create a PowerVS Workspace](https://cloud.ibm.com/docs/power-iaas?topic=power-iaas-creating-power-virtual-server). The steps are:
   ...
   1. In the catalog's search box, type `Power Virtual Server` and click **Workspace for Power Virtual Server** tile.
   ...
   1. Click **Agree**.
   1. Click **Create**.
![image](https://github.com/IBM-Cloud/vpc-transit/assets/6932057/8fcd658c-3331-454c-a1ef-57c47e16b100)
...
1. Click **Subnets** under **Networking** in the left navigation.
   ...
   1. Click **Create subnet**
1. Click **Virtual server instances** under **Compute** in the left navigation.
...
Note the **IPs** address for the virtual server instance just created. It is the IP on the private subnet and will be referred to as $PPIP (Power Private IP) later in the tutorial. Something like the following, but your IP will be different.
...
](https://cloud.ibm.com/docs/solution-tutorials?topic=solution-tutorials-vpc-transit-power) and learn how to connect using the high performance networking of the IBM Cloud with the privacy and security that meet your requirements. It also covers connectivity to fully managed services like [IBM Cloud Databases](https://cloud.ibm.com/docs/cloud-databases) and [Cloud Object Storage](https://cloud.ibm.com/docs/cloud-object-storage?topic=cloud-object-storage-getting-started-cloud-object-storage).